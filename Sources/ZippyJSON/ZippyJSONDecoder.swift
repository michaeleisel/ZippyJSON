...
private var _iso8601Formatter: JJLISO8601DateFormatter = {
...
extension Dictionary: DictionaryWithoutKeyConversion
...
public final class ZippyJSONDecoder {
  @available(
    *,
    deprecated,
    message:
      "This flag is deprecated because full-precision parsing speed is now on par with imprecise, so it will just always use full-precision"
  )
  ...
  private func createContext(string: UnsafePointer<Int8>, length: Int)
    -> ContextPointer
  {
  ...
  public func decode<T: Decodable>(_ type: T.Type, from data: Data) throws -> T
  {
    if isOnSimulator() && !JNTHasVectorExtensions() {
      return try decodeWithAppleDecoder(
        type,
        from: data,
        reason:
          "This library was not compiled with the necessary vector extensions (this is likely because you're using SwiftPM + the simulator, and is due to limitations with SwiftPM. This does not apply to real devices.)"
      )
    }
    if case .custom(_) = keyDecodingStrategy {
      return try decodeWithAppleDecoder(
        ...
        from: data,
        reason:
          "Custom key decoding is not supported, because it is uncommon and makes efficient parsing difficult"
      )
    }
    return try data.withUnsafeBytes { (bytes) -> T in
      ...
      if success {
        ...
        // nicer.
        let retryReasonString =
          retryReason.map { String(utf8String: $0)! } ?? ""
        return try decodeWithAppleDecoder(
          type,
          from: data,
          reason: retryReasonString
        )
      }
    }
  }

  func decodeWithAppleDecoder<T: Decodable>(
    _ type: T.Type,
    from data: Data,
    reason: String?
  ) throws -> T {
    let appleDecoder = Foundation.JSONDecoder()
    appleDecoder.dataDecodingStrategy =
      ZippyJSONDecoder.convertDataDecodingStrategy(dataDecodingStrategy)
    appleDecoder.dateDecodingStrategy =
      ZippyJSONDecoder.convertDateDecodingStrategy(dateDecodingStrategy)
    appleDecoder.keyDecodingStrategy =
      ZippyJSONDecoder.convertKeyDecodingStrategy(keyDecodingStrategy)
    appleDecoder.nonConformingFloatDecodingStrategy =
      ZippyJSONDecoder.convertNonConformingFloatDecodingStrategy(
        nonConformingFloatDecodingStrategy
      )
    appleDecoder.userInfo = userInfo
    if !ZippyJSONDecoder.zjd_suppressWarnings {
      print(
        "[ZippyJSONDecoder] Warning: fell back to using Apple's JSONDecoder. Reason: \(reason ?? ""). This message will only be printed the first time this happens. To suppress this message entirely, for all reasons, use `ZippyJSONDecoder.zjd_suppressWarnings = true"
      )
  ...
  static func convertNonConformingFloatDecodingStrategy(
    _ strategy: ZippyJSONDecoder.NonConformingFloatDecodingStrategy
  ) -> Foundation.JSONDecoder.NonConformingFloatDecodingStrategy {
    switch strategy {
  ...
  static func convertDateDecodingStrategy(
    _ strategy: ZippyJSONDecoder.DateDecodingStrategy
  ...
  static func convertDataDecodingStrategy(
    _ strategy: ZippyJSONDecoder.DataDecodingStrategy
  ) -> Foundation.JSONDecoder.DataDecodingStrategy {
    switch strategy {
    case .base64:
  ...
  static func convertKeyDecodingStrategy(
    _ strategy: ZippyJSONDecoder.KeyDecodingStrategy
  ) -> Foundation.JSONDecoder.KeyDecodingStrategy {
    switch strategy {
...
@inline(__always) private func swiftErrorFromError(
  _ context: ContextPointer,
  ...
  case .jsonParsingFailed:
    error = DecodingError.dataCorrupted(
      DecodingError.Context(codingPath: [], debugDescription: debugDescription)
    )
  case .none:
    fallthrough
  @unknown default:
...
final private class __JSONDecoder: Decoder {
  ...
  let nonConformingFloatDecodingStrategy:
    ZippyJSONDecoder.NonConformingFloatDecodingStrategy
  ...
  init(
    value: Value,
    containers: JSONDecodingStorage,
    keyDecodingStrategy: ZippyJSONDecoder.KeyDecodingStrategy,
    dataDecodingStrategy: ZippyJSONDecoder.DataDecodingStrategy,
    dateDecodingStrategy: ZippyJSONDecoder.DateDecodingStrategy,
    nonConformingFloatDecodingStrategy: ZippyJSONDecoder
      .NonConformingFloatDecodingStrategy,
  ...
  > where Key: CodingKey {
    ...
    return KeyedDecodingContainer(
      try JSONKeyedDecoder<Key>(
        decoder: self,
        value: containers.topContainer,
    ...
    if JNTDocumentValueIsDouble(value) {
      ...
      throw DecodingError.typeMismatch(
        ...
        DecodingError.Context(
          ...
          debugDescription:
            "Expected to decode Decimal but found incorrect type instead"
        )
      )
    }
  }

  fileprivate func unbox(_ value: Value, as type: Decimal.Type) throws
    -> Decimal
  {
  ...
  fileprivate func unbox(_ value: Value, as type: Date.Type, key: CodingKey?)
    throws -> Date
  {
  ...
  fileprivate func unbox(_ value: Value, as type: Data.Type, key: CodingKey?)
    throws -> Data
  {
    push(container: value, key: key)
    defer { pop(shouldRemoveKey: key != nil) }
  ...
  fileprivate func unbox<T>(
    _ value: Value,
    as type: DictionaryWithoutKeyConversion.Type,
    key codingKey: CodingKey?
  ) throws -> T {
    var result = [String: Any]()
    var innerError: Error?
    JNTDocumentForAllKeyValuePairs(
      value,
      { key, subValue in
    ...
    if let resultCasted = result as? T {
      ...
      throw DecodingError.dataCorrupted(
        DecodingError.Context(
          codingPath: [],
          debugDescription: "Bad dictionary"
        )
      )
    }
  }
...
extension __JSONDecoder {
  // UnboxBegin
  @inline(__always) fileprivate func unbox(
    _ value: Value,
    as type: UInt8.Type,
    key: CodingKey?
  ) throws -> UInt8 {
  ...
  fileprivate func unboxNestedUnkeyedContainer(value: Value) throws
  ...
  fileprivate func unboxSuper(_ value: Value, key: CodingKey?) -> Decoder {
    ...
    return __JSONDecoder(
      value: value,
      containers: containers.createCopy(),
      keyDecodingStrategy: keyDecodingStrategy,
      dataDecodingStrategy: dataDecodingStrategy,
      dateDecodingStrategy: dateDecodingStrategy,
  ...
  fileprivate func unboxNestedContainer<NestedKey>(
    value: Value,
    keyedBy type: NestedKey.Type
  ) throws -> KeyedDecodingContainer<NestedKey> where NestedKey: CodingKey {
    return KeyedDecodingContainer(
      try JSONKeyedDecoder<NestedKey>(
        decoder: self,
        value: value,
        convertToCamel: convertToCamel
      )
...
private struct JSONUnkeyedDecoder: UnkeyedDecodingContainer {
  ...
  mutating func decode<T>(_ type: T.Type) throws -> T where T: Decodable {
    currentValue = try valueFromIterator()
    let decoded = try decoder.unbox(
      currentValue,
      as: type,
      key: IndexKey(index: currentIndex)
    )
    advanceArray()
    return decoded
  }
    ...
    guard JNTDocumentValueIsArray(value) else {
      throw DecodingError.typeMismatch(
        ...
        DecodingError.Context(
          ...
          debugDescription: "Tried to unbox array, but it wasn't an array"
        )
      )
    }
  }

  mutating public func nestedContainer<NestedKey>(keyedBy type: NestedKey.Type)
    throws -> KeyedDecodingContainer<NestedKey>
  {
    currentValue = try valueFromIterator()
    let container = try decoder.unboxNestedContainer(
      value: currentValue,
      keyedBy: type
    )
    advanceArray()
  ...
  mutating func superDecoder() throws -> Decoder {
    currentValue = try valueFromIterator()
    let container = decoder.unboxSuper(
      currentValue,
      key: IndexKey(index: currentIndex)
    )
    advanceArray()
  ...
  // UnkeyedBegin
  @inline(__always) public mutating func decode(_ type: UInt8.Type) throws
    -> UInt8
  {
    currentValue = try valueFromIterator()
    let decoded = try decoder.unbox(
  ...
  @inline(__always) public mutating func decode(_ type: UInt16.Type) throws
    -> UInt16
  {
    currentValue = try valueFromIterator()
    let decoded = try decoder.unbox(
  ...
  @inline(__always) public mutating func decode(_ type: UInt32.Type) throws
    -> UInt32
  {
    currentValue = try valueFromIterator()
    let decoded = try decoder.unbox(
  ...
  @inline(__always) public mutating func decode(_ type: UInt64.Type) throws
    -> UInt64
  {
    currentValue = try valueFromIterator()
    let decoded = try decoder.unbox(
  ...
  @inline(__always) public mutating func decode(_ type: Int8.Type) throws
    -> Int8
  {
    currentValue = try valueFromIterator()
    let decoded = try decoder.unbox(
  ...
  @inline(__always) public mutating func decode(_ type: Int16.Type) throws
    -> Int16
  {
    currentValue = try valueFromIterator()
    let decoded = try decoder.unbox(
  ...
  @inline(__always) public mutating func decode(_ type: Int32.Type) throws
    -> Int32
  {
    currentValue = try valueFromIterator()
    let decoded = try decoder.unbox(
  ...
  @inline(__always) public mutating func decode(_ type: Int64.Type) throws
    -> Int64
  {
    currentValue = try valueFromIterator()
    let decoded = try decoder.unbox(
  ...
  @inline(__always) public mutating func decode(_ type: Bool.Type) throws
    -> Bool
  {
    currentValue = try valueFromIterator()
    let decoded = try decoder.unbox(
  ...
  @inline(__always) public mutating func decode(_ type: String.Type) throws
    -> String
  {
    currentValue = try valueFromIterator()
    let decoded = try decoder.unbox(
  ...
  @inline(__always) public mutating func decode(_ type: Double.Type) throws
    -> Double
  {
    currentValue = try valueFromIterator()
    let decoded = try decoder.unbox(
  ...
  @inline(__always) public mutating func decode(_ type: Float.Type) throws
    -> Float
  {
    currentValue = try valueFromIterator()
    let decoded = try decoder.unbox(
  ...
  @inline(__always) public mutating func decode(_ type: UInt.Type) throws
    -> UInt
  {
    currentValue = try valueFromIterator()
    let decoded = try decoder.unbox(
...
@inline(__always)
private func throwErrorIfNecessary(
  _ value: Value,
  decoder: __JSONDecoder,
  key: CodingKey?
) throws {
  guard !JNTDocumentErrorDidOccur(value) else {
    let error = swiftErrorFromError(
      JNTGetContext(value),
      decoder: decoder,
      key: key
    )
...
private final class JSONKeyedDecoder<K: CodingKey>:
  KeyedDecodingContainerProtocol
{
    ...
    guard JNTDocumentValueIsDictionary(value) else {
      throw DecodingError.typeMismatch(
        ...
        DecodingError.Context(
          codingPath: [],
          debugDescription:
            "Tried to unbox dictionary, but it wasn't a dictionary"
        )
      )
    }
  }

  fileprivate static func setupValue(
    _ value: Value,
    decoder: __JSONDecoder,
    convertToCamel: Bool
  ) throws -> Value {
  ...
  fileprivate init(decoder: __JSONDecoder, value: Value, convertToCamel: Bool)
    throws
  {
    try self.value = JSONKeyedDecoder<K>.setupValue(
      value,
      decoder: decoder,
      convertToCamel: convertToCamel
    )
  ...
  @inline(__always) private func fetchValue(keyPointer: UnsafePointer<Int8>)
    throws -> Value
  {
  ...
  // KeyedBegin
  @inline(__always) fileprivate func decode(_ type: UInt8.Type, forKey key: K)
    throws -> UInt8
  {
    let subValue: Value = try key.stringValue.withCString(fetchValue)
  ...
  @inline(__always) fileprivate func decode(_ type: UInt16.Type, forKey key: K)
    throws -> UInt16
  {
    let subValue: Value = try key.stringValue.withCString(fetchValue)
  ...
  @inline(__always) fileprivate func decode(_ type: UInt32.Type, forKey key: K)
    throws -> UInt32
  {
    let subValue: Value = try key.stringValue.withCString(fetchValue)
  ...
  @inline(__always) fileprivate func decode(_ type: UInt64.Type, forKey key: K)
    throws -> UInt64
  {
    let subValue: Value = try key.stringValue.withCString(fetchValue)
  ...
  @inline(__always) fileprivate func decode(_ type: Int8.Type, forKey key: K)
    throws -> Int8
  {
    let subValue: Value = try key.stringValue.withCString(fetchValue)
  ...
  @inline(__always) fileprivate func decode(_ type: Int16.Type, forKey key: K)
    throws -> Int16
  {
    let subValue: Value = try key.stringValue.withCString(fetchValue)
  ...
  @inline(__always) fileprivate func decode(_ type: Int32.Type, forKey key: K)
    throws -> Int32
  {
    let subValue: Value = try key.stringValue.withCString(fetchValue)
  ...
  @inline(__always) fileprivate func decode(_ type: Int64.Type, forKey key: K)
    throws -> Int64
  {
    let subValue: Value = try key.stringValue.withCString(fetchValue)
  ...
  @inline(__always) fileprivate func decode(_ type: Bool.Type, forKey key: K)
    throws -> Bool
  {
    let subValue: Value = try key.stringValue.withCString(fetchValue)
  ...
  @inline(__always) fileprivate func decode(_ type: String.Type, forKey key: K)
    throws -> String
  {
    let subValue: Value = try key.stringValue.withCString(fetchValue)
  ...
  @inline(__always) fileprivate func decode(_ type: Double.Type, forKey key: K)
    throws -> Double
  {
    let subValue: Value = try key.stringValue.withCString(fetchValue)
  ...
  @inline(__always) fileprivate func decode(_ type: Float.Type, forKey key: K)
    throws -> Float
  {
    let subValue: Value = try key.stringValue.withCString(fetchValue)
  ...
  @inline(__always) fileprivate func decode(_ type: Int.Type, forKey key: K)
    throws -> Int
  {
    let subValue: Value = try key.stringValue.withCString(fetchValue)
  ...
  @inline(__always) fileprivate func decode(_ type: UInt.Type, forKey key: K)
    throws -> UInt
  {
    let subValue: Value = try key.stringValue.withCString(fetchValue)
  ...
  @inline(__always) fileprivate func decode<T: Decodable>(
    _ type: T.Type,
    forKey key: K
  ) throws -> T {
    let subValue: Value = try key.stringValue.withCString(fetchValue)
